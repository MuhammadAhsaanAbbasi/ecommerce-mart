...
async def sign_up_admin(admin: Admin, session: DB_SESSION):
    admin = create_admin(admin, session)
    return admin


@admin_router.post("/login", response_model=Token)
async def login_access_token_admin(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], session: DB_SESSION):
...
async def reset_password(reset_password:str, current_user: Annotated[Admin, Depends(get_current_admin)], session:DB_SESSION):
    if current_user:
...
async def update_user(current_user: Annotated[Admin, Depends(get_current_active_admin_user)], user:UserBase, session:DB_SESSION):
    if current_user:
