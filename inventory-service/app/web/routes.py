...
async def create_product_sizes(current_admin: Annotated[Admin, Depends(get_current_active_admin_user)],
                        session: DB_SESSION,
...
async def update_product_item_sizes(
    ...
    session: DB_SESSION,
    product_size_detail: ProductItemFormModel,
    product_size_id: int,
    image: Optional[UploadFile] = File(None)
):
    product_item = await update_product_item_size(current_admin, session, product_size_detail, product_size_id, image)
    return {"message": "Size of Product Updated Successfully!", "data": product_item}